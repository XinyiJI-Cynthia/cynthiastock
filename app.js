...
async function loadNews() {
  const tasks = state.stocks.flatMap((stock) => [
    fetchStockNews(stock, "domestic"),
    fetchStockNews(stock, "global")
  ]);
  ...
  if (errors.length) {
    state.errors.push(...Array.from(new Set(errors)).slice(0, 3));
  }

  state.items = dedupeArticles(articles).sort((a, b) => b.timestamp - a.timestamp);
}

async function fetchStockNews(stock, sourceBucket) {
  const query = buildNewsQuery(stock, sourceBucket);
  const directUrl = new URL(GDELT_ENDPOINT);
  directUrl.searchParams.set("query", query);
  directUrl.searchParams.set("mode", "ArtList");
  directUrl.searchParams.set("format", "json");
  ...
  const articles = Array.isArray(data.articles) ? data.articles : [];
  return articles
    .map((article) => normalizeArticle(article, stock, sourceBucket))
    .filter((article) => article.title && article.url)
    .filter((article) => relevanceScore(article, stock) > 0)
    .map((article) => ({ ...article, signal: classifyArticle(article, stock) }))
    .slice(0, 18);
}
...
function buildNewsQuery(stock, sourceBucket) {
  const terms = sourceBucket === "domestic"
    ? stock.domesticTerms
    : stock.globalTerms;
  const compactTerms = [stock.name, stock.code, ...terms]
    .map((term) => String(term || "").trim())
    .filter(Boolean);
  return Array.from(new Set(compactTerms)).slice(0, 6).map(quoteGdeltTerm).join(" OR ");
}
