...
这是一个手机优先的静态网页，用来查看自选股票的公开行情、国内外新闻信息，并对每条信息进行 AI 研判。
...
- `index.html`：网页入口。
- `styles.css`：移动端优先样式。
- `app.js`：行情、新闻读取、关注列表和 AI 研判结果展示。
...
- AI 研判：GitHub Actions 使用 GitHub Models 的 `openai/gpt-4.1`，逐条生成性质、置信度、影响范围、判断依据、影响路径和观察建议。
- 公司公告入口：默认指向交易所公司资料或公告入口。

AI 只能依据新闻标题、来源和时间分析，不补写标题中没有的事实。页面会区分公司基本面、行业信息、市场交易信号和无关信息；员工持股计划、ESG、估值指标等不会自动视为利好。AI 暂时不可用时，页面会明确显示“待判断”或规则后备标签。

页面给出的建议属于非个性化观察建议，不包含确定买卖指令、目标价或收益预测。重要事项应继续核对上交所公告和公司官方信息。
